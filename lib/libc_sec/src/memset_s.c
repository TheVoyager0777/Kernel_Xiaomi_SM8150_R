...
#include <boot.h>
#elif defined(__XLOADER_SEC_LIB__) || defined(__UCE_SEC_LIB__)
#include "string.h"
#endif
...
#if defined(__LPMCU__)
__ao_func
#endif
errno_t memset_s(void* dest, size_t destMax, int c, size_t count)
{
    if (SECUREC_LIKELY(count <= destMax  && dest  && destMax <= SECUREC_MEM_MAX_LEN ))
    {
...
{
    if (SECUREC_LIKELY(count <= destMax  && dest ))
    {
        if (count > SECURE_MEMSET_THRESHOLD_SIZE)
        {
#ifdef USE_ASM
